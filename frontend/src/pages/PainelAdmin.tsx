...
import { getAgendamentos, getProfissionais, atualizarStatusAgendamento } from '../services/api'
...
function PainelAdmin() {
  ...
  const receitaTotal = agendamentos
    ...
    .reduce((acc, a) => acc + Number(a.servico_preco), 0)
    ...
    <div style={{ minHeight: '100vh', background: '#0d0d0d', fontFamily: "'Inter', sans-serif", color: '#f0f0f0' }}>
      ...
      <div style={{ background: '#111', borderBottom: '0.5px solid #1e1e1e', borderLeft: '3px solid #e63c2f', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ ...bc, fontSize: '11px', color: '#555', letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>Painel do Administrador</div>
        </div>
      ...
      <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        {/* Dashboard */}
        <div>
          <p style={{ ...bc, fontSize: '10px', color: '#e63c2f', letterSpacing: '0.12em', textTransform: 'uppercase' as const, fontWeight: '600', marginBottom: '0.75rem' }}>Dashboard</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px', marginBottom: '8px' }}>
            <div style={{ background: '#111', border: '0.5px solid #1e1e1e', borderRadius: '2px', padding: '14px', textAlign: 'center' }}>
              <div style={{ ...bc, fontSize: '28px', fontWeight: '800', color: '#e63c2f' }}>R$ {receitaTotal.toFixed(2)}</div>
              <div style={{ ...bc, fontSize: '10px', color: '#555', letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>Receita confirmada</div>
            </div>
            <div style={{ background: '#111', border: '0.5px solid #1e1e1e', borderRadius: '2px', padding: '14px', textAlign: 'center' }}>
              <div style={{ ...bc, fontSize: '28px', fontWeight: '800', color: '#e63c2f' }}>{total}</div>
              <div style={{ ...bc, fontSize: '10px', color: '#555', letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>Total agendamentos</div>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
            {[{ num: confirmados, label: 'Confirmados', color: '#4caf7d' }, { num: pendentes, label: 'Pendentes', color: '#c9a84c' }, { num: cancelados, label: 'Cancelados', color: '#e63c2f' }].map(s => (
              <div key={s.label} style={{ background: '#111', border: '0.5px solid #1e1e1e', borderRadius: '2px', padding: '14px', textAlign: 'center' }}>
                <div style={{ ...bc, fontSize: '22px', fontWeight: '800', color: s.color }}>{s.num}</div>
        ...
        {/* Equipe */}
        <div>
          <p style={{ ...bc, fontSize: '10px', color: '#e63c2f', letterSpacing: '0.12em', textTransform: 'uppercase' as const, fontWeight: '600', marginBottom: '0.75rem' }}>Equipe</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', maxHeight: '240px', overflowY: 'auto', paddingRight: '8px' }}>
            {profissionais.map(p => {
              const atendimentos = agendamentos.filter(a => a.profissional_nome === p.nome && a.status !== 'cancelado').length
              return (
                <div key={p.id} style={{ background: '#111', border: '0.5px solid #1e1e1e', borderLeft: '2px solid #e63c2f', borderRadius: '2px', padding: '14px' }}>
                  <div style={{ width: '32px', height: '32px', borderRadius: '2px', background: '#e63c2f22', border: '0.5px solid #e63c2f44', display: 'flex', alignItems: 'center', justifyContent: 'center', ...bc, fontSize: '12px', fontWeight: '700', color: '#e63c2f', marginBottom: '8px' }}>{p.iniciais}</div>
                  <div style={{ ...bc, fontSize: '13px', fontWeight: '700', color: '#f0f0f0', textTransform: 'uppercase' as const, marginBottom: '4px' }}>{p.nome}</div>
                  <div style={{ fontSize: '11px', color: '#555', marginBottom: '6px' }}>{atendimentos} atendimentos</div>
                  <div style={{ fontSize: '10px', color: '#888' }}>Salário: R$ {Number(p.salario).toFixed(2)}</div>
                </div>
              )
            })}
          </div>
        ...
        {/* Agendamentos */}
        <div>
          <p style={{ ...bc, fontSize: '10px', color: '#e63c2f', letterSpacing: '0.12em', textTransform: 'uppercase' as const, fontWeight: '600', marginBottom: '0.75rem' }}>Agendamentos</p>
